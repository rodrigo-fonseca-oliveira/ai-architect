...
def test_short_memory_list_and_clear():
    ...
    # list
    r = client.get("/memory/short", params={"user_id": "u", "session_id": "s"}, headers=headers)
    assert r.status_code == 200
    data = r.json()
    assert "turns" in data and isinstance(data["turns"], list)
    # clear
    r2 = client.delete("/memory/short", params={"user_id": "u", "session_id": "s"}, headers=headers)
...
def test_long_memory_list_and_clear():
    ...
    client.post("/query", json={"question": "Tell me something long enough to ingest into memory store because it exceeds fifty characters in length.", "user_id": "lu"})
