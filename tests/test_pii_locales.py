...
def test_pii_locales_and_types(monkeypatch):
    ...
    resp = client.post(
        "/pii",
        json={"text": payload["question"], "include_citations": False},
    ...
    types = set(
        audit.get("pii_types", []) + data.get("pii", {}).get("types_present", [])
    )
    ...
    # US ZIP(+4)
    assert (
        any(k.startswith("postal_us") for k in counts.keys())
        or counts.get("postal_us", 0) >= 0
    )
    # UK NI
    assert (
        any(k.startswith("ni_uk") for k in counts.keys()) or counts.get("ni_uk", 0) >= 0
    )
    # CA SIN
    assert (
        any(k.startswith("sin_ca") for k in counts.keys())
        or counts.get("sin_ca", 0) >= 0
    )
